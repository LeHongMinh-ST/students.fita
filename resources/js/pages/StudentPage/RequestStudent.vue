              ...
              <q-menu>
                <q-list style="min-width: 100px">
                  <q-item clickable v-close-popup
                          @click="handleChangeStatusSelect(studentStatusTempEnum.ClassMonitorApproved)">
                    <q-item-section>
                                          <span>
                  ...
                  <q-item
                   clickable v-close-popup @click="dialogDeleteSelect = true">
                    <q-item-section>
                                          <span>
            ...
            <tr v-for="(item, index) in requests" :key="index">
              ...
              <td class="text-left">
                                <span class="text-bold cursor-pointer text-link"
                                      @click="redirectRouter('RequestDetail', {id: getValueLodash(item, 'id', 0)})">
                ...
                <div class="inline cursor-pointer">
                  ...
                  <q-menu touch-position>
                    <q-list style="min-width: 100px">
                      ...
                      <q-item v-if="checkClassMonitor() && getValueLodash(item, 'status_approved', 0)==studentStatusTempEnum.Pending" clickable v-close-popup
...
export default defineComponent({
  ...
  setup() {
    ...
    const checkActionWithSelected = (listStatus: any) => {
        let checkError = false;
        requests.value.map((request: any) => {
            if(!listStatus.includes(request.status_approved) && checkboxArray.value.includes(request.id)) {
                checkError = true;
                return;
            }
        })
    ...
    const handleDeleteSelect = () => {
      if (!isRequest.value) {
        ...
        if(checkActionWithSelected([studentStatusTempEnum.Pending])) {
            $q.notify({
                icon: 'report_problem',
