        ...
        <q-dialog v-model="dialogImportDialog" @hide="closeDialog">
            <q-card style="width: 500px;">
                ...
                <q-card-section style="width: 100%">
                    <div class="q-mb-sm">
                        Tải: <span @click="downloadExcelTemplate" class="cursor-pointer text-link"
                                   style="color: #1976D2">tệp mẫu</span>
                    </div>
    ...
    setup() {
        ...
        const getStudentByCondition = async (payload = {}, variable: any) => {
            try {
                ...
                variable.value = _.get(res, 'data.data.students', '')
            } catch (error) {
                generateNotify("Không tải được dữ liệu lớp học !")
            }
        }
        ...
        const handleUploadExcel = (): void => {
            if (!file.value) {
                generateNotify('Vui lòng chọn tệp', false)
                return
            }
