...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      if (idReport.value) {
        handleGetReport(idReport.value);
      }
    })
    ...
    const handleGetReport = (id: string): void => {
      $q.loading.show()
      api.getReportStudent<{}>(id).then(res => {
        ...
        $q.notify({
          icon: 'report_problem',
          message: 'Không tải được dữ liệu phản ánh !',
          color: 'negative',
          position: 'top-right'
        })
    ...
    const handleCreateReport = () => {
      ...
      if (!isRequest.value) {
        ...
        api.updateStudentReport(data, idReport.value).then(res => {
          if (res) {
